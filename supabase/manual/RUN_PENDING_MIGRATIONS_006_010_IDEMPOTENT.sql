-- ===========================================================================
-- RUN_PENDING_MIGRATIONS_006_010_IDEMPOTENT.sql
-- ===========================================================================
-- Corrected, idempotent, transactional version of RUN_PENDING_MIGRATIONS_006_010.sql,
-- produced after an explicit audit of that file. Same effect, same objects, same seed
-- data - nothing added, nothing removed, nothing destructive in either version. Two
-- changes only, both purely defensive:
--
--   1. Wrapped in BEGIN/COMMIT so the whole thing is one atomic unit - if anything
--      fails partway through, nothing is left half-applied.
--   2. Every `create table`/`create index`/`create policy` that came from migration
--      010 (which, unlike 006/008, wasn't written with re-run safety in mind) now has
--      the same "if not exists" / "drop policy if exists" guard the rest of the file
--      already used - so running this twice by accident is a no-op the second time,
--      not an error partway through.
--
-- Still safe to run on the current production state (001-005 applied, nothing from
-- 006 onward) - and now also safe if you have to re-run it for any reason.
-- ===========================================================================
...
-- ---------------------------------------------------------------------------
-- Only structural difference from the original 010: every `create table`/`create index` below
-- gained `if not exists`, and every `create policy` gained a `drop policy if exists` right before
-- it - migration 010 was the one file in this batch not written with a second run in mind. Table
-- shapes, FKs, RLS logic and grants are otherwise byte-identical to the original.
